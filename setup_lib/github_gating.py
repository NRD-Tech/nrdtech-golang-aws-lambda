...
def resolve_github_approval_settings(args, current, non_interactive):
    ...
    if mode not in constants.APPROVAL_MODES:
        print("Unknown approval mode '{}'; using {}".format(mode, constants.DEFAULT_APPROVAL_MODE), file=sys.stderr)
...
def enable_github_workflow():
    if os.path.isfile(constants.WORKFLOW_ENABLED):
        print("GitHub workflow already enabled at {}".format(constants.WORKFLOW_ENABLED))
        return True
    if not os.path.isfile(constants.WORKFLOW_DISABLED):
        print("Warning: {} not found".format(constants.WORKFLOW_DISABLED), file=sys.stderr)
        return False
    os.rename(constants.WORKFLOW_DISABLED, constants.WORKFLOW_ENABLED)
    print("Enabled GitHub workflow: {}".format(constants.WORKFLOW_ENABLED))
    return True
